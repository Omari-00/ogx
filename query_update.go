...
func (q *UpdateQuery) mustAppendSet(fmter schema.Formatter, b []byte) (_ []byte, err error) {
	...
	case *sliceTableModel:
		return nil, errors.New("ogx: to bulk Update, use CTE and VALUES")
	default:
		return nil, fmt.Errorf("ogx: Update does not support %T", q.tableModel)
	}
...
func (q *UpdateQuery) Bulk() *UpdateQuery {
	...
	if !ok {
		q.setErr(fmt.Errorf("ogx: Bulk requires a slice, got %T", q.model))
		return q
	}
