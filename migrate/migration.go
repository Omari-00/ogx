...
type Migration struct {
	...
	Name       string
	GroupID    int64
	MigratedAt time.Time `ogx:",notnull,nullzero,default:current_timestamp"`

	Up   MigrationFunc `ogx:"-"`
	Down MigrationFunc `ogx:"-"`
}
...
type MigrationFunc func(ctx context.Context, db *ogx.DB) error

func NewSQLMigrationFunc(fsys fs.FS, name string) MigrationFunc {
	return func(ctx context.Context, db *ogx.DB) error {
		...
		for scanner.Scan() {
			...
			if bytes.HasPrefix(b, []byte(prefix)) {
				...
				return fmt.Errorf("ogx: unknown directive: %q", b)
			}
		...
		defer func() {
			if tx, ok := idb.(ogx.Tx); ok {
				retErr = tx.Commit()
				return
			}

			if conn, ok := idb.(ogx.Conn); ok {
				retErr = conn.Close()
				return
			}
