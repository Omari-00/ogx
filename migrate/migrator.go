...
func NewMigrator(db *ogx.DB, migrations *Migrations, opts ...MigratorOption) *Migrator {
	m := &Migrator{
		db:         db,
		migrations: migrations,
		...
		table:      "ogx_migrations",
		locksTable: "ogx_migration_locks",
	}
...
func (m *Migrator) formattedTableName(db *ogx.DB) string {
...
func (m *Migrator) Unlock(ctx context.Context) error {
	...
	_, err := m.db.NewDelete().
		Model((*migrationLock)(nil)).
		ModelTableExpr(m.locksTable).
		Where("? = ?", ogx.Ident("table_name"), tableName).
		Exec(ctx)
	return err
}
