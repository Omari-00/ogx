...
//
//    Attrs  map[string]string `ogx:",hstore"`
func HStore(vi interface{}) *HStoreValue {
	v := reflect.ValueOf(vi)
	if !v.IsValid() {
		panic(fmt.Errorf("ogx: HStore(nil)"))
	}
	...
	if typ.Kind() != reflect.Map {
		panic(fmt.Errorf("ogx: Hstore(unsupported %s)", typ))
	}

	return &HStoreValue{
		v: v,

		append: ogdialect.hstoreAppender(v.Type()),
...
func (h *HStoreValue) AppendQuery(fmter schema.Formatter, b []byte) ([]byte, error) {
	if h.append == nil {
		panic(fmt.Errorf("ogx: HStore(unsupported %s)", h.v.Type()))
	}
...
func (h *HStoreValue) Scan(src interface{}) error {
	if h.scan == nil {
		return fmt.Errorf("ogx: HStore(unsupported %s)", h.v.Type())
	}
	if h.v.Kind() != reflect.Ptr {
		return fmt.Errorf("ogx: HStore(non-pointer %s)", h.v.Type())
	}
