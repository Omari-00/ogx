...
import (
	...
	"github.com/niconical/ogx/internal/tagparser"
)
...
func (t *Table) CheckPKs() error {
	if len(t.PKs) == 0 {
		return fmt.Errorf("ogx: %s does not have primary keys", t)
	}
	return nil
}
...
func (t *Table) Field(name string) (*Field, error) {
	...
	if !ok {
		return nil, fmt.Errorf("ogx: %s does not have column=%s", t, name)
	}
	return field, nil
}
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if f.Anonymous {
			...
			if fieldType.Kind() == reflect.Struct {
				...
				tag := tagparser.Parse(f.Tag.Get("ogx"))
...
func (t *Table) newField(f reflect.StructField, prefix string, index []int) *Field {
	...
	if nextPrefix, ok := tag.Option("embed"); ok {
		fieldType := indirectType(f.Type)
		if fieldType.Kind() != reflect.Struct {
			panic(fmt.Errorf("ogx: embed %s.%s: got %s, wanted reflect.Struct",
...
func (t *Table) initRelation(field *Field, rel string) {
	...
	default:
		panic(fmt.Errorf("ogx: unknown relation=%s on field=%s", rel, field.GoName))
	}
}
...
func (t *Table) belongsToRelation(field *Field) *Relation {
	...
	if onUpdate, ok := field.Tag.Options["on_update"]; ok {
		if len(onUpdate) > 1 {
			panic(fmt.Errorf("ogx: %s belongs-to %s: on_update option must be a single field", t.TypeName, field.GoName))
		}

		rule := strings.ToUpper(onUpdate[0])
		if !isKnownFKRule(rule) {
			internal.Warn.Printf("ogx: %s belongs-to %s: unknown on_update rule %s", t.TypeName, field.GoName, rule)
		}
	...
	if onDelete, ok := field.Tag.Options["on_delete"]; ok {
		if len(onDelete) > 1 {
			panic(fmt.Errorf("ogx: %s belongs-to %s: on_delete option must be a single field", t.TypeName, field.GoName))
		}

		rule := strings.ToUpper(onDelete[0])
		if !isKnownFKRule(rule) {
			internal.Warn.Printf("ogx: %s belongs-to %s: unknown on_delete rule %s", t.TypeName, field.GoName, rule)
		}
	...
	if join, ok := field.Tag.Options["join"]; ok {
		...
		for i, baseColumn := range baseColumns {
			...
			if f := t.fieldWithLock(baseColumn); f != nil {
				...
				panic(fmt.Errorf(
					"ogx: %s belongs-to %s: %s must have column %s",
	...
	if join, ok := field.Tag.Options["join"]; ok {
		...
		for i, baseColumn := range baseColumns {
			if f := t.fieldWithLock(baseColumn); f != nil {
				...
				panic(fmt.Errorf(
					"ogx: %s has-one %s: %s must have column %s",
...
func (t *Table) hasManyRelation(field *Field) *Relation {
	...
	if field.IndirectType.Kind() != reflect.Slice {
		panic(fmt.Errorf(
			"ogx: %s.%s has-many relation requires slice, got %q",
	...
	if join, ok := field.Tag.Options["join"]; ok {
		...
		for i, baseColumn := range baseColumns {
			...
			if f := t.fieldWithLock(baseColumn); f != nil {
				...
				panic(fmt.Errorf(
					"ogx: %s has-many %s: %s must have column %s",
					t.TypeName, field.GoName, t.TypeName, baseColumn,
				))
			}
	...
	if isPolymorphic {
		...
		if rel.PolymorphicField == nil {
			panic(fmt.Errorf(
				"ogx: %s has-many %s: %s must have polymorphic column %s",
...
func (t *Table) m2mRelation(field *Field) *Relation {
	if field.IndirectType.Kind() != reflect.Slice {
		panic(fmt.Errorf(
			"ogx: %s.%s m2m relation requires slice, got %q",
			t.TypeName, field.GoName, field.IndirectType.Kind(),
		))
	}
	joinTable := t.dialect.Tables().Ref(indirectType(field.IndirectType.Elem()))
	...
	if !ok {
		panic(fmt.Errorf("ogx: %s must have m2m tag option", field.GoName))
	}

	m2mTable := t.dialect.Tables().ByName(m2mTableName)
	if m2mTable == nil {
		panic(fmt.Errorf(
			"ogx: can't find m2m %s table (use db.RegisterModel)",
			m2mTableName,
		))
	}
	...
	if leftField == nil {
		panic(fmt.Errorf(
			"ogx: %s many-to-many %s: %s must have field %s "+
