...
import (
	...
	"github.com/niconical/ogx/dialect/sqltype"
	"github.com/niconical/ogx/extra/ogxjson"
...
func scanJSON(dest reflect.Value, src interface{}) error {
	...
	return ogxjson.Unmarshal(b, dest.Addr().Interface())
}
...
func scanJSONUseNumber(dest reflect.Value, src interface{}) error {
	...
	dec := ogxjson.NewDecoder(bytes.NewReader(b))
	dec.UseNumber()
...
func scanIP(dest reflect.Value, src interface{}) error {
	...
	if ip == nil {
		return fmt.Errorf("ogx: invalid ip: %q", b)
	}
...
func addrScanner(fn ScannerFunc) ScannerFunc {
	return func(dest reflect.Value, src interface{}) error {
		if !dest.CanAddr() {
			return fmt.Errorf("ogx: Scan(nonaddressable %T)", dest.Interface())
		}
...
func toBytes(src interface{}) ([]byte, error) {
	...
	case []byte:
		return src, nil
	default:
		return nil, fmt.Errorf("ogx: got %T, wanted []byte or string", src)
	}
}
...
func scanError(dest reflect.Type, src interface{}) error {
	return fmt.Errorf("ogx: can't scan %#v (%T) into %s", src, src, dest.String())
}
