...
// IDB is a common interface for *ogx.DB, ogx.Conn, and ogx.Tx.
type IDB interface {
	IConn
...
func (q *baseQuery) checkSoftDelete() error {
	...
	if q.tableModel == nil {
		return errors.New("ogx: can't use soft deletes without a table model")
	}
	return nil
}
...
func (q *baseQuery) excludeColumn(columns []string) {
	...
	for _, column := range columns {
		if !q._excludeColumn(column) {
			q.setErr(fmt.Errorf("ogx: can't find column=%q", column))
			return
		}
	}
}
...
func (q *baseQuery) _appendFirstTable(
	...
	return nil, errors.New("ogx: query does not have a table")
}
...
func (q *whereBaseQuery) addWhereCols(cols []string) {
	if q.table == nil {
		err := fmt.Errorf("ogx: got %T, but WherePK requires a struct or slice-based model", q.model)
		q.setErr(err)
		return
	}
	if q.whereFields != nil {
		err := errors.New("ogx: WherePK can only be called once")
		q.setErr(err)
		return
	}
...
func (q *whereBaseQuery) mustAppendWhere(
	...
	if len(q.where) == 0 && q.whereFields == nil {
		err := errors.New("ogx: Update and Delete queries require at least one Where")
		return nil, err
	}
...
func (q *whereBaseQuery) appendWhereFields(
	...
	if q.table == nil {
		err := fmt.Errorf("ogx: got %T, but WherePK requires struct or slice-based model", q.model)
		return nil, err
	}
	...
	default:
		return nil, fmt.Errorf("ogx: WhereColumn does not support %T", q.tableModel)
	}
}
