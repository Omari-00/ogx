...
func (h *QueryHook) BeforeQuery(
	ctx context.Context, event *ogx.QueryEvent,
) context.Context {
	return ctx
}

func (h *QueryHook) AfterQuery(ctx context.Context, event *ogx.QueryEvent) {
	if !h.enabled {
		return
	}
...
func formatOperation(event *ogx.QueryEvent) string {
